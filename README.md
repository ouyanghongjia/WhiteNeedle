...
| Path | Description |
|------|-------------|
| `ios-dylib/` | WhiteNeedle dynamic library source and `Makefile` |
...
| `resign-tool/` | IPA resign / inject helper scripts |
...
- iOS **15.0+** (arm64)
- macOS with Xcode / iOS SDK to build the dylib
...
```bash
cd ios-dylib && make
# Output: ios-dylib/build/WhiteNeedle.dylib
```
