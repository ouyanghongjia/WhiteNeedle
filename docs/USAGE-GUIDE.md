...
如果 App 的 `Info.plist` 正确配置了 Bonjour（重签名方式自动配置；CocoaPods 方式需在 Podfile 中添加 hook），设备会自动出现在列表中。
...
模块安装到设备沙盒的 `Library/wn_installed_modules/` 目录中。WhiteNeedle 的模块加载器会按以下顺序搜索 `require()` 的模块：

```
1. Library/wn_modules/        （内置模块）
2. Library/wn_installed_modules/ （用户安装的模块）
...
| 类型声明异常 | 删除 `.vscode/whiteneedle.d.ts`，重新加载窗口让扩展重新生成 |
