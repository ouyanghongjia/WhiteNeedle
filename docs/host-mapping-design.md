...
WhiteNeedle 作为注入到 iOS App 进程中的动态调试工具（Framework），其 Host Mapping 功能的目标是：**将指定域名的网络请求重定向到用户配置的目标 IP**，类似于桌面端 SwitchHosts 工具。
...
4. **VPN/NetworkExtension 不可用**：虽然 `NEPacketTunnelProvider` 能在内核层拦截全设备流量，但它需要独立 `.appex` 进程 + 特殊 entitlement + 用户显式授权，无法从注入的 Framework 中实现
...
将 Host Mapping 和 Mock 两个职责**彻底分离**，Host Mapping 完全由 VSCode Extension 侧的代理服务器处理，iOS 端不保留任何 Host Mapping 代码：
...
| **请求监控**（请求/响应明文展示） | App (WhiteNeedle) → WebSocket → VSCode Extension | 应用层主动上报（非代理抓包） |
...
| 请求监控 | 需额外实现 | **✅ WhiteNeedle 主动上报明文数据** |
| Host Mapping 代码在 App 端 | ✅（大量 hook 代码） | **❌（零 host mapping 代码）** |
...
`NEPacketTunnelProvider` 能在内核 NECP 层拦截全设备所有 App 的流量，但无法从注入的 Framework 中使用：
...
| `WhiteNeedle.m` | Framework 入口，初始化各组件 |
...
| 文件 | 原职责 | 删除原因 |
|------|--------|----------|
| `WNHostMapping.h/m` | DNS/connect hook、NSURLProtocol URL 改写、WKWebView 桥接、Host Mapping 规则管理 | Host Mapping 完全由外部代理处理，Framework 中不再需要任何 host mapping 逻辑 |
...
**v6 的关键决策**：
- Host Mapping 从 Framework 中完全移除，交由 Mac 端代理处理
